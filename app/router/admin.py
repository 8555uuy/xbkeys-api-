...
@router.post("/cards/unbind")
def admin_unbind_card(
    request: Request,
    card_id: int = Form(...),
    db: Session = Depends(get_db),
):
    """解绑指定卡密：将已使用的卡密恢复为未使用，并清除设备绑定信息。"""
    if not _admin(request, db):
        return redirect_with("/login", "需要管理员权限", "error")
    card = db.query(Card).filter(Card.id == card_id).first()
    if not card:
        return redirect_with("/admin/cards", "卡密不存在", "error")
    if card.status != "used":
        return redirect_with("/admin/cards", "仅已使用的卡密可解绑", "error")
    card.status = "unused"
    card.used_at = None
    card.used_ip = ""
    card.used_ua = ""
    card.used_device = ""
    add_log(db, get_current_user(request, db), "unbind", f"后台解绑卡密 {card.code}", client_ip(request))
    db.commit()
    return redirect_with("/admin/cards", f"卡密 {card.code} 已解绑")


@router.post("/cards/toggle")
def admin_toggle_card(
    request: Request,
    card_id: int = Form(...),
    db: Session = Depends(get_db),
):
    ...
    if not card:
        return redirect_with("/admin/cards", "卡密不存在", "error")
    if card.status == "used":
        return redirect_with("/admin/cards", "已使用的卡密不可停用，请先解绑", "error")
    card.status = "unused" if card.status == "disabled" else "disabled"
    add_log(db, get_current_user(request, db), "toggle_card",
            f"{'启用' if card.status == 'unused' else '停用'}卡密 {card.code}", client_ip(request))
    db.commit()
    return redirect_with("/admin/cards",
                         f"卡密 {card.code} 已{'启用' if card.status == 'unused' else '停用'}")
...
def export_cards(request: Request, db: Session = Depends(get_db)):
    ...
    for c in q.all():
        writer.writerow([
            ...
            "已停用" if c.status == "disabled" else "已使用" if c.status == "used" else "未使用",
