...
def _login_user(db: Session, user: User, msg: str) -> RedirectResponse:
    """创建会话并写入登录 Cookie。"""
...
def register(
    ...
    db.add(user)
    db.commit()
    add_log(db, user, "register", "注册账号", client_ip(request))
...
def login(
    ...
    add_log(db, user, "login", "用户登录", client_ip(request))
